...
describe('abstractor', function() {
	...
	describe('when called', function() {
		...
		it('should always return an array', function() {
			expect(abstractor()).to.be.instanceof(Array);
		});

		describe('with no template', function() {

			it('should return an empty array', function() {
				expect(abstractor()).to.have.property('length', 0);
			});
	...
	describe('when template contains', function() {
		...
		describe('a single element', function() {
			...
			it('should return an array with correct number of items', function() {
				expect(abstractor('<div></div>')).to.have.property('length', 2);
			});

			it('should return an array containing object equivalients for each element', function() {
				expect(abstractor('<div></div>')[0].type).to.equal('div');
				expect(abstractor('<div></div>')[0].close).to.equal(undefined);
				expect(abstractor('<div></div>')[1].type).to.equal('div');
				expect(abstractor('<div></div>')[1].close).to.equal(true);
				expect(abstractor('<p></p>')[0].type).to.equal('p');
			});
		...
		describe('a tagName containing a number', function() {
			...
			it('should return an array with correct number of items', function() {
				expect(abstractor(tpl)).to.have.property('length', 2);
			});

			it('should return an array containing object equivalients for each element', function() {
				expect(abstractor(tpl)[0].type).to.equal('h1');
				expect(abstractor(tpl)[0].close).to.equal(undefined);
				expect(abstractor(tpl)[1].type).to.equal('h1');
				expect(abstractor(tpl)[1].close).to.equal(true);
			});
		...
		describe('an element containing a text node', function() {
			...
			it('should return an array with correct number of items', function() {
				expect(abstractor(tpl)).to.have.property('length', 3);
			});

			it('should return an array containing object equivalients for each element', function() {
				expect(abstractor(tpl)[0].type).to.equal('label');
				expect(abstractor(tpl)[0].close).to.equal(undefined);
				expect(abstractor(tpl)[1].type).to.equal('#text');
				expect(abstractor(tpl)[1].value).to.equal('Hello');
				expect(abstractor(tpl)[2].type).to.equal('label');
				expect(abstractor(tpl)[2].close).to.equal(true);
			});
